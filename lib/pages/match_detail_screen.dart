...
import '../utils/mock_defensive_data.dart';
import '../utils/mock_lineup_data.dart';
import '../utils/mock_match_events.dart';
...
import '../widgets/tabs/enhanced_events_tab.dart';
...
class _MatchDetailScreenState extends State<MatchDetailScreen>
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: widget.match.isScheduled ? 5 : 7, vsync: this);
  ...
  // Wrapper sui mock estratti in mock_shot_data.dart e mock_defensive_data.dart
  // Quando arrivera l'API, questo metodo sara sostituito da chiamate API
  // che ritornano shot e defensive data reali per il match_id corrente.
  void _loadShotData() {
    _homeShotsData = generateMockHomeShots();
    _awayShotsData = generateMockAwayShots();
    _homeDefensiveActions = generateMockHomeDefensiveActions();
    _awayDefensiveActions = generateMockAwayDefensiveActions();
  }
  ...
  void _loadStandings() {
    setState(() {
      _standingsFuture = ApiService().fetchStandings(135);
    });
  }
  ...
  Widget _buildAttackView(bool isDark) {
    return AttackView(
      homeTeamName: widget.match.homeTeamName,
      awayTeamName: widget.match.awayTeamName,
      showHome: _advancedStatsShowHome,
      timeFilter: _attackTimeFilter,
      mockAttackMomentum: _mockAttackMomentum,
      inSelectedHalf: _inSelectedHalf,
      generateEvents: _generateDetailedMockEvents,
    );
  }
  ...
  Widget _buildHeatmapView(bool isDark) {
    return HeatmapView(
      ...
      homeCorners: _homeCorners,
      homeFinalThirdEntries: _homeFinalThirdEntries,
      homeHeatZones: _homeHeatZones,
      homePassesCompleted: _homePassesCompleted,
      homePassesTotal: _homePassesTotal,
      homePossAttack: _homePossAttack,
      homePossDefense: _homePossDefense,
      homePossMidfield: _homePossMidfield,
      homePossession: _homePossession,
      homeShotsOnTarget: _homeShotsOnTarget,
      homeShotsTotal: _homeShotsTotal,
      ...
      awayCorners: _awayCorners,
      awayFinalThirdEntries: _awayFinalThirdEntries,
      awayHeatZones: _awayHeatZones,
      awayPassesCompleted: _awayPassesCompleted,
      awayPassesTotal: _awayPassesTotal,
      awayPossAttack: _awayPossAttack,
      awayPossDefense: _awayPossDefense,
      awayPossMidfield: _awayPossMidfield,
      awayPossession: _awayPossession,
      awayShotsOnTarget: _awayShotsOnTarget,
      awayShotsTotal: _awayShotsTotal,
      awayTerritory: _awayTerritory,
      heatGridCols: _heatGridCols,
      heatGridRows: _heatGridRows,
  ...
  // [FAV-extract-mock-events] wrapper su generateMockMatchEvents
  ...
  Widget _buildEnhancedEventsTab(ThemeData theme, bool isDark) {
    return EnhancedEventsTab(
      ...
      localizeEventDetail: _localizeEventDetail,
      openPlayerStatsFromEvent: _openPlayerStatsFromEvent,
      showPlayerMatchStats: _showPlayerMatchStats,
    );
  }
  ...
  Widget _buildLineupsTab(ThemeData theme, bool isDark) {
    return LineupsTab(
      ...
      generateBench: _generateMockBench,
      buildTeamSelector: _buildTeamSelector3,
      showCoachProfile: _showCoachProfile,
      showPlayerMatchStats: _showPlayerMatchStats,
    );
  }
  ...
  // [FAV-extract-mock-lineup] wrapper su generateMockBench
  // (mock estratto in mock_lineup_data.dart)
  List<LocalLineupPlayer> _generateMockBench(bool isHome) {
    return generateMockBench(isHome);
  }
  ...
  // [FAV-extract-match-notif-dialog] wrapper sul dialog estratto
  ...
  Future<void> _showMatchNotificationDialog() async {
    await showMatchNotificationDialogV2(
      context,
      match: widget.match,
      getSettings: () => _matchNotifSettings,
      setSettings: (s) {
        setState(() {
          _matchNotifSettings = s;
        });
      },
      service: _matchNotifService,
      haptic: _haptic,
  ...
  // ============================================================================
  // [FAV-extract-mock-lineup] wrapper su generateMockLineup
  // (mock estratto in mock_lineup_data.dart)
  // Quando arrivera l'API, sostituire con chiamata API.
  List<LocalLineupPlayer> _generateMockLocalLineup(bool isHome) {
    return generateMockLineup(isHome);
  }
}
